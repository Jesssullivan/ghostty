...
pub fn add(
    ...
    // Glad (OpenGL loader) is needed for both exe and lib builds when
    // using the OpenGL renderer. The embedded apprt's surfaceInit calls
    // prepareContext() which uses gladLoaderLoadGLContext.
    if (self.config.renderer == .opengl) {
